...
function newAvg($arr, $newavg) {
    $new_total = $newavg * (count($arr) + 1);
    $next_donation = $new_total - array_sum($arr);
    if ($next_donation <= 0) throw new InvalidArgumentException;
    return ceil($next_donation);
}

// function newAvg($arr, $newavg) {
//     $amount = ceil(((count($arr) + 1) * $newavg) - array_sum($arr));
//     if($amount < 0) throw new InvalidArgumentException();
//     return $amount;
// }

// function newAvg(array $arr, $newavg): int
// {
//   $all  = $newavg * (count($arr) + 1);
//   $next = $all - array_sum($arr);
//   if ($next <= 0) {
