...
In some cases, it may be impractical to define a class and a class method, only to instantiate exactly one object from that class just to use a particular method. 
If that is the case, you are better off with static class methods. 
Static class methods are class methods that can be directly accessed through the class without the creation of a new instance of that class, and the syntax to define it is as follows:
class MyClass {
  /* Class code here */
...
The static keyword is placed between the visibility declaration and the function definition, and it tells PHP that, "Hey, I want to access this particular method directly from the class without creating an object from it!". 
It should also be possible to set the visibility of a static method to protected or private but I haven't tested it yet so you are free to test it in your local servers in your spare time.
...
//   CurrentUSPresident::greet('Donald');