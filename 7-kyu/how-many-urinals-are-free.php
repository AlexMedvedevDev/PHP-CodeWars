...
function GetFreeUrinals($urinals){
    if(strpos($urinals, '11') !== false) return -1;
    return preg_match_all('/(^|0)0(?!1)/', $urinals);
}

// function GetFreeUrinals($urinals){
//     if (strstr($urinals, '11')) return -1;
//     $count1 = 0;
//     if (substr($urinals, 0, 2) == '00') {$count1++; $urinals[0] = '1';}
//     $haystack = $urinals;
//     $needle = '000';
//     while (strpos($haystack, $needle) != false) {
//       $count1++;
//       $urinals = substr_replace($haystack, '010', strpos($haystack, $needle), strlen($needle));
//       $haystack = $urinals;
//     }
//     if (substr($urinals, -2) == '00') {$count1++;};
//     return $count1;
// }

// function GetFreeUrinals($urinals) {
//     if (preg_match("/11/", $urinals)) {
//       return -1;
//     }
//     preg_match_all("/(?<!1)0+(?!1)/", $urinals, $matches);
//     return array_sum(array_map(function ($e) {
//         return ceil(strlen($e) / 2);
//       }, $matches[0]));
// }